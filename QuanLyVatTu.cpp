...
void inKhoanThoiGian(Date ngayBatDau, Date ngayKetThuc) {
	...
	TextColor(7);
	gotoxy(62, 6); cout << " Tu Ngay : " << ngayBatDau.ngay << "/" << ngayBatDau.thang << "/" << ngayBatDau.nam;
	gotoxy(62, 7); cout << " Den Ngay: " << ngayKetThuc.ngay << "/" << ngayKetThuc.thang << "/" << ngayKetThuc.nam;
}
...
long long tinhThanhTienCTHD(ChiTietHoaDon cthd) {
	...
	long long thanhTienChuaThue = cthd.soLuong * (long long)(cthd.donGia);
	float tienThue = (thanhTienChuaThue * cthd.VAT) / 100;
	return thanhTienChuaThue + tienThue;
}
...
void themMotDoanhThu(DSDoanhThu &DSDT, DoanhThu *doanhThu) {
	...
	{
		THONG_BAO(" DANH SACH THONG KE DOANH THU DAY! ", 90, 40, 79);
		return;
	}
	...
	for(int i = 0; i < DSNV.sl; i++) 
	{
		for(PTRHD p = DSNV.nv[i]->DSHD.FirstHD; p != NULL; p = p->pNext) 
		{
			...
			if(p->info.loai == "X" && ngayNamTrongKhoang(ngayBatDau, ngayKetThuc, p->info.ngayLap)) 
			{
				...
				{
					thanhTien = tinhThanhTienCTHD(p->info.dsChiTietHoaDon.ds[j]);
...
void xuatTopDoanhThu(DSDoanhThu &DSDT, Date ngayBatDau, Date ngayKetThuc) {
	if(DSDT.sl == 0)
	{
		THONG_BAO("KHONG CO DOANH THU TRONG KHOANG THOI GIAN DA NHAP", 90, 40, 79);
		return;
	}
...
NhapNgayBD:
	gotoxy(80, 13); ngayBatDau.ngay = nhapSoNguyen();  if(ngayBatDau.ngay == -1) { xoaKhungNhapXemTGHD(); THONG_BAO("BAN DA THOAT XEM TOP DOANH THU", 90, 40, 79); return; }
	gotoxy(80, 14); ngayBatDau.thang = nhapSoNguyen(); if(ngayBatDau.thang == -1) { xoaKhungNhapXemTGHD(); THONG_BAO("BAN DA THOAT XEM TOP DOANH THU", 90, 40, 79); return; }
	gotoxy(80, 15); ngayBatDau.nam = nhapSoNguyen();   if(ngayBatDau.nam == -1) { xoaKhungNhapXemTGHD(); THONG_BAO("BAN DA THOAT XEM TOP DOANH THU", 90, 40, 79); return; }
...
NhapNgayKT:
	gotoxy(112, 13); ngayKetThuc.ngay = nhapSoNguyen();  if(ngayKetThuc.ngay == -1)  { xoaKhungNhapXemTGHD(); THONG_BAO("BAN DA THOAT XEM TOP DOANH THU", 90, 40, 79); return; }
	gotoxy(112, 14); ngayKetThuc.thang = nhapSoNguyen(); if(ngayKetThuc.thang == -1) { xoaKhungNhapXemTGHD(); THONG_BAO("BAN DA THOAT XEM TOP DOANH THUN", 90, 40, 79); return; }
	gotoxy(112, 15); ngayKetThuc.nam = nhapSoNguyen();   if(ngayKetThuc.nam == -1) { xoaKhungNhapXemTGHD(); THONG_BAO("BAN DA THOAT XEM TOP DOANH THU", 90, 40, 79); return; }
...
NhapLai:
	...
	if(!trungMaVT) {
		...
		khungCapNhatNV();
		TextColor(222);
		gotoxy(91, 20); cout << " THONG TIN VAT TU CAP NHAT ";
		TextColor(7);
		...
		gotoxy(102, 23); tenVTCN = nhapLieu(1, trungMaVT->info.tenVT);
		if(tenVTCN == "-1") {
			THONG_BAO(" BAN DA THOAT CAP NHAT VAT TU! ", 90, 40, 79);
			return;
		}
		
		gotoxy(102, 24); dvtCN = nhapLieu(1, trungMaVT->info.dvt);
		if(tenVTCN == "-1") {
			THONG_BAO(" BAN DA THOAT CAP NHAT VAT TU! ", 90, 40, 79);
			return;
		}
...
void sapXepNhanVien(DSNhanVien &DSNV)
{
	for (int i = 0; i < DSNV.sl - 1; i++)
	{
		...
		{
			if (DSNV.nv[i]->ten.compare(DSNV.nv[j]->ten) > 0)
			{
				hoanViHaiNV(DSNV.nv[i], DSNV.nv[j]);
			}
			else if (DSNV.nv[i]->ten.compare(DSNV.nv[j]->ten) == 0)
			{
				if (DSNV.nv[i]->ho.compare(DSNV.nv[j]->ho) > 0)
				{
					hoanViHaiNV(DSNV.nv[i], DSNV.nv[j]);
				}
			}
...
NhapLai:
	...
	if (vt < 0) {
		...
		gotoxy(102, 22); hoCN = nhapLieu(2, DSNV.nv[vt]->ho);
		if(hoCN == "-1") {
			THONG_BAO(" BAN DA THOAT CAP NHAT NHAN VIEN! ", 90, 40, 79);
			return;
		}
		
		gotoxy(102, 23); tenCN = nhapLieu(2, DSNV.nv[vt]->ten);
		if(hoCN == "-1") {
			THONG_BAO(" BAN DA THOAT CAP NHAT NHAN VIEN! ", 90, 40, 79);
			return;
		}
		...
		if(DSNV.nv[vt]->phai.compare("NU") == 0)
			chon = 1;
...
PTRHD ktTrungMaHDTrongNV(PTRHD hoaDon, string soHD) {
...
void khungThongTinCTHD(DSChiTietHoaDon DSCTHD, DSVatTu DSVT){
	TextColor(241);
	gotoxy(147, 5); cout << "                                              ";
	...
	gotoxy(205, 32); cout << char(188);
	
	gotoxy(133, 9); cout << char(201);
	gotoxy(133, 11); cout << char(204);
	gotoxy(133, 32); cout << char(200);
	...
	gotoxy(140, 32); cout << char(202);
	gotoxy(164, 32); cout << char(202);
	gotoxy(181, 32); cout << char(202);
	gotoxy(193, 32); cout << char(202);
	
	gotoxy(135, 10); cout << "STT";
	gotoxy(147, 10); cout << "TEN VAT TU";
	...
	int stt = 0, hang = 0;
	for(int i = 0; i < DSCTHD.sl; i++) {
		TREE vatTu = timNodeVT(DSVT.TREE_VT, DSCTHD.ds[i].maVT);
		
		stt += 1;
		gotoxy(137, 12 + hang); cout << stt; 
		gotoxy(145, 12 + hang); cout << vatTu->info.tenVT;
...
void capNhatDSSLTon(DSVatTu &DSVT, DSChiTietHoaDon &DSCTHD, string loai){
	for(int i = 0; i < DSCTHD.sl;  i++) {
...
// ============================= cap nhat so luong ton cua mot vat tu =============================================
void capNhatMotSLTon(DSVatTu &DSVT, ChiTietHoaDon CTHD, string loai){
	TREE tree = timNodeVT(DSVT.TREE_VT, CTHD.maVT);
	// neu hoa don xuat thi khi cap nhat lai phai cong vao so vat tu da xuat ra
	if(tree && loai.compare("X") == 0) 
	{ 
		tree->info.slTon += CTHD.soLuong;
		tree->info.tonTaiTrongHD -= 1;
	}
	// neu hoa don nhap thi khi cap nhat lai phai tru di so vat tu da nhap
	else if(tree && loai.compare("N") == 0) 
...
bool xacNhanThoatLapHD(DSNhanVien &DSNV, DSVatTu &DSVT, PTRHD p, int vtNVLap, string loai) {
	...
	if(xacNhan) {
		...
		if(p->info.dsChiTietHoaDon.sl > 0) {
			...
			if(xacNhan) {
				...
				capNhatDSSLTon(DSVT, p->info.dsChiTietHoaDon, loai);
...
NhapLaiNgay:
		...
		if(soSanhHaiNgay == -1) {
			THONG_BAO(" NGAY LAP HOA DON PHAI NHO HON HOAC BANG NGAY HIEN TAI! ", 90, 40, 79);
			xoaNhapNgayBatDau();
			goto NhapLaiNgay;
		}
...
NhapCTHDNhap:
			...
			khungThongTinCTHD(p->info.dsChiTietHoaDon, DSVT);
			...
			if(CTHD.maVT == "-1") { // khi nguoi dung nhan ESC
				bool kiemTraThoat = xacNhanThoatLapHD(DSNV, DSVT, p, vtNVLap, p->info.loai);
				if(!kiemTraThoat)
					goto NhapCTHDNhap;
				else {
				...
				goto XoaCTHDNhap;
			} else if(CTHD.maVT == "-3") { // khi nguoi dung nha F1
				goto XuatDSVTNhap;
			}
			...
			if(!tree) {
				...
				} else if(CTHD.soLuong == -2) { // khi nguoi dung nha DELETE
					goto XoaCTHDNhap;
				}
				...
				} else if(CTHD.donGia == -2) { // khi nguoi dung nha DELETE
					goto XoaCTHDNhap;
				}
				...
				} else if(CTHD.VAT == -2) { // khi nguoi dung nha DELETE
					goto XoaCTHDNhap;
				}
				
				int vtCTHD = timCTHD(p->info.dsChiTietHoaDon, CTHD.maVT);
				if(vtCTHD < 0) { // neu ma CTHD chua ton tai trong danh sach chi tiet hoa don
					...
					p->info.dsChiTietHoaDon.ds[vtCTHD].soLuong += CTHD.soLuong;
					TextColor(224);
					gotoxy(65, 40); cout << " SAN PHAM DA TON TAI TRONG HOA DON! HE THONG SE TU DONG CAP NHAT SO LUONG, LAY DON GIA VA VAT MAC DINH CUA SAN PHAM ";
					TextColor(7);
					Sleep(5000);
					gotoxy(90, 40); cout << "                                                                                                                    ";
					goto NhapCTHDNhap;
...
XoaCTHDNhap:
			khungXoaCTHD();
			gotoxy(86, 35); cout << "NHAP MA VAT TU BAN MUON XOA: ";
			string maVTXoa;
			gotoxy(84, 37); cout << "Ma Vat Tu: "; maVTXoa = nhapLieu(3, noiDungCu);
			if(maVTXoa == "-1") {
				xoaKhungXoaCTHD();
				goto NhapCTHDNhap;
			}
			
			int viTriCTHD = timCTHD(p->info.dsChiTietHoaDon, maVTXoa);
			if(viTriCTHD < 0) {
				THONG_BAO("MA VAT TU BAN VUA NHAP KHONG CO TRONG HOA DON!", 90, 40, 79);
				noiDungCu = maVTXoa;
				goto XoaCTHDNhap;
			} else {
				capNhatMotSLTon(DSVT, p->info.dsChiTietHoaDon.ds[viTriCTHD], p->info.loai);
				xoaMotCTHD(p->info.dsChiTietHoaDon, viTriCTHD);
				//xoaMotCTHD(DSCTHDTam, viTriCTHD);
				THONG_BAO("XOA CHI TIET HOA DON THANH CONG!", 90, 40, 167);
				xoaKhungXoaCTHD();
				goto NhapCTHDNhap;
			}
...
void inDSHDMotNhanVien(DSNhanVien &DSNV, int vtNV) {
	...
	while(1) {
		if(DSNV.nv[vtNV]->DSHD.sl < 15) {
			tongTrang = 1;
		} else {
			tongTrang = (DSNV.nv[vtNV]->DSHD.sl - 1) / 15 + 1;
		}
		
		// dua vao bien duyet de biet in ra tu phan tu nao
		int hang = 0, stt = 0, bienDuyet = 0;
		...
		for (PTRHD p = DSNV.nv[vtNV]->DSHD.FirstHD; p != NULL; p = p->pNext) {
			if((bienDuyet >= gioiHanDuyetTrai && bienDuyet < gioiHanDuyetPhai) && bienDuyet < DSNV.nv[vtNV]->DSHD.sl) {
				stt++;
				gotoxy(81, 12 + hang); cout << stt;
				gotoxy(91, 12 + hang); cout << p->info.soHD;
				gotoxy(113, 12 + hang); cout << p->info.ngayLap.ngay << "/" << p->info.ngayLap.thang << "/" << p->info.ngayLap.nam;
				gotoxy(157, 12 + hang); cout << tenLoaiHD(p->info.loai);
				hang++;
				bienDuyet++;
			} else {
				bienDuyet++;
...
void inChiTietHD(PTRHD p, string soHD, DSVatTu DSVT) {
	if(p->info.dsChiTietHoaDon.sl == 0)
	{
		TextColor(79);
	...
	{
		khungThongTinHD();
		gotoxy(62, 11); cout << "So Hoa Don : " << p->info.soHD;
		gotoxy(62, 12); cout << "Ngay Lap   : " << p->info.ngayLap.ngay << "/" << p->info.ngayLap.thang << "/" << p->info.ngayLap.nam;
		gotoxy(62, 13); cout << "Loai       : " << tenLoaiHD(p->info.loai);
		...
		while(1) {
			...
			xoaKhungXuatCTHD(); khungXuatCTHD();
			...
			TextColor(175);
			gotoxy(180, 35); cout << " TONG CONG: " << tongCong << " ";
			TextColor(7);
...
void sapXepHoaDon(DSHoaDon &DSHD) {
	// neu khong co hoa don nao hoac chi co 1 hoa don thi return luon tranh truong hop NULL->pNext
	if(DSHD.sl <= 1)
		return;
	...
	for(p = DSHD.FirstHD; p->pNext != NULL; p = p->pNext) {
		pMax = p;
		hoaDonMax = p->info;
		for(q = p->pNext; q != NULL; q = q->pNext) {
			if(soSanhThoiGian(pMax->info.ngayLap, q->info.ngayLap) == -1) { // neu ngay a < ngay b
				pMax = q;
				hoaDonMax = q->info;
			}
		}
...
XuatDSHD:		
		sapXepHoaDon(DSNV.nv[vtNV]->DSHD);
...
NhapSoHD:
		...
		PTRHD tonTaiSoHD = ktTrungMaHDTrongNV(DSNV.nv[vtNV]->DSHD.FirstHD, soHD);
		if(!tonTaiSoHD) 
		{
		...
		else 
		{
			...
			inChiTietHD(tonTaiSoHD, soHD, DSVT);
			return;
		}
	}
XuatDSNV:
...
// ham dua vao so hoa don va duyet tra ve vi tri nhan vien so huu hoa don do
...
void giaiPhongHD(DSHoaDon &DSHD) {
	for(PTRHD p = DSHD.FirstHD; p!= NULL; p = p->pNext)
		delete p;
}

void inThongTinHoaDon(DSHoaDon &DSHD, DSNhanVien &DSNV, Date ngayBatDau, Date ngayKetThuc, DSHoaDon &DSHDTam) {
	if(DSHD.sl == 0) {
...
NhapNgayBD:
	gotoxy(80, 13); ngayBatDau.ngay = nhapSoNguyen();  if(ngayBatDau.ngay == -1) { xoaKhungNhapXemTGHD(); THONG_BAO("BAN DA THOAT XEM HOA DON", 90, 40, 79); return; }
	gotoxy(80, 14); ngayBatDau.thang = nhapSoNguyen(); if(ngayBatDau.thang == -1) { xoaKhungNhapXemTGHD(); THONG_BAO("BAN DA THOAT XEM HOA DON", 90, 40, 79); return; }
	gotoxy(80, 15); ngayBatDau.nam = nhapSoNguyen();   if(ngayBatDau.nam == -1) { xoaKhungNhapXemTGHD(); THONG_BAO("BAN DA THOAT XEM HOA DON", 90, 40, 79); return; }
	bool trangThaiNBD = laNgayHopLe(ngayBatDau);
	if(!trangThaiNBD)
	{
		THONG_BAO("NGAY BAT DAU KHONG TON TAI!", 90, 40, 79);
...
NhapNgayKT:
	gotoxy(112, 13); ngayKetThuc.ngay = nhapSoNguyen();  if(ngayKetThuc.ngay == -1)  { xoaKhungNhapXemTGHD(); THONG_BAO("BAN DA THOAT XEM HOA DON", 90, 40, 79); return; }
	gotoxy(112, 14); ngayKetThuc.thang = nhapSoNguyen(); if(ngayKetThuc.thang == -1) { xoaKhungNhapXemTGHD(); THONG_BAO("BAN DA THOAT XEM HOA DON", 90, 40, 79); return; }
	gotoxy(112, 15); ngayKetThuc.nam = nhapSoNguyen();   if(ngayKetThuc.nam == -1) { xoaKhungNhapXemTGHD(); THONG_BAO("BAN DA THOAT XEM HOA DON", 90, 40, 79); return; }
	bool trangThaiNKT = laNgayHopLe(ngayKetThuc);
	if(!trangThaiNKT)
	{
		THONG_BAO("NGAY KET THUC KHONG TON TAI!", 90, 40, 79);
	...
	sapXepHoaDon(DSHDTam);
	inThongTinHoaDon(DSHDTam, DSNV, ngayBatDau, ngayKetThuc, DSHDTam);
}
